...
class financeAction extends baseAction
{
	...
	*/
	public function index()
	{
		...
		$data = $this->loadModel('xwjr')->getList('*',$where,'id desc',true);
	...
	{
		$id = intval(I('id'));
		$rs = $this->loadModel('xwjr')->getInfoByid($id);
		$this->assign('rs',$rs);		
		$this->display('view');
	}
	...
	public function save()
	{
		...
		$rs = !$id?$this->loadModel('xwjr')->add($data):$this->loadModel('xwjr')->update($data,array('id'=>$id));
	...
	*/
	public function delete()
	{
		...
		$rs = $this->loadModel('xwjr')->del(array('id'=>array('IN',$ids)));
		$this->ajaxOut($rs,'index');
	}
}
