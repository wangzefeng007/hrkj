...
function flog($content,$tag='',$file = '')
{

	if (!defined('C_LOG_PATH'))
	{
		throw_exception("Please set LogPath!");
	}
	if (is_array($content))
	{
		ob_start();
	...
	$file = $file?$file:date('Ymd');
	$log_file = C_LOG_PATH.$file.".log";
		
	$log = "\n[".date("YmdHis")."]:".$tag.":".$content;
	$f = fopen($log_file,"ab+");
	fwrite($f,$log."\n");
	fclose($f);
}