 ...
 */
function xml2string($key, $node) {
    if (is_array($node)) {
        $xml = $key ? '<' . $key.'>': '';
        if(!is_hash_array($node)){
            $str = '';
            foreach ( $node as $leaf ) {
                $str .= '<item>'.xml2string('', $leaf).'</item>';
            }
        }else{
            $str = '';
            foreach ( $node as $k => $leaf ) {
                $str .= xml2string($k, $leaf);
            }
        }
        return $xml ? $xml . $str . ($key ? '</' . $key . '>' :'') : $str;

    } else {
        // 大量字符则使用cdata，少量用htmlspecialchars编码
        return $key ? '<' . $key . '>' . (is_numeric($node) || empty($node) ? $node : (isset($node{500}) ? '<![CDATA[' . $node . ']]>' : htmlspecialchars($node))) . '</' . $key . '>' : $node;
    }
}
...
function logger($str){
    $log=str_replace("\n","",$str)."\n";
    error_log('['.date('Y-m-d H:i:s',time()).']'.$log,3,ROOT.'/data/'.date('ymd').'.log');
}