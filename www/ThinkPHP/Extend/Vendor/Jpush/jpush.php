...
class jpush
{  
    private $app_key = 'e0904d065a85ea92759cef3d';            //待发送的应用程序(appKey)，只能填一个。
    private $master_secret = 'e117d2fa869e0ac02f89a2d3';        //主密码
    ...
    */
    ...
    public function push($receiver='all',$content='',$title='',$m_type='',$m_txt='',$m_time='86400',$content_message='')
    {
		...
		if(!$content_message) 
		{
			$content_message=array('type'=>3,'content'=>$content);
			$type_index = $content_message['type'];
		}
		if(!is_array($content_message))
		{
			$content_type = json_decode($content_message,true);
			$type_index = $content_type['type'];
		}
        if($content)
        {
	        $data['notification'] = array(
	            ...
	            "ios"=>array(
	                ...
	                "badge"=>"+1",
	            	'sound' => 'default',
	                "extras"=>array("type"=>$type_index, "txt"=>$m_txt)
	            ),
	        );
        }
        $data['message'] = array(
		...
		//print_R(json_decode($param,true));exit;
