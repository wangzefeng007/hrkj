...
class AlipayNotify {
	...
	function verifyNotify(){
		if(empty($_POST)) {//判断POST来的数组是否为空
			...
			if ($this->alipay_config['sign_type'] == '0001') {
				$decrypt_post_para['notify_data'] = AlipayCommon::rsaDecrypt($decrypt_post_para['notify_data'], $this->alipay_config['private_key_path']);
			}
	...
	function decrypt($prestr) {
		return AlipayCommon::rsaDecrypt($prestr, trim($this->alipay_config['private_key_path']));
	}
	...
	function getSignVeryfy($para_temp, $sign, $isSort) {
		//除去待签名参数数组中的空值和签名参数
		$para = AlipayCommon::paraFilter($para_temp);
		
		//对待签名参数数组排序
		if($isSort) {
			$para = AlipayCommon::argSort($para);
		} else {
			$para = $this->sortNotifyPara($para);
		}
		
		//把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
		$prestr = AlipayCommon::createLinkstring($para);
		
		$isSgin = false;
		switch (strtoupper(trim($this->alipay_config['sign_type']))) {
			case "MD5" :
				$isSgin = AlipayCommon::md5Verify($prestr, $sign, $this->alipay_config['key']);
				break;
			case "RSA" :
				$isSgin = AlipayCommon::rsaVerify($prestr, trim($this->alipay_config['ali_public_key_path']), $sign);
				break;
			case "0001" :
				$isSgin = AlipayCommon::rsaVerify($prestr, trim($this->alipay_config['ali_public_key_path']), $sign);
				break;
	...
	function getResponse($notify_id) {
		...
		$responseTxt = AlipayCommon::getHttpResponseGET($veryfy_url, $this->alipay_config['cacert']);
