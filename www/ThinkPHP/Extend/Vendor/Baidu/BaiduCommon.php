...
class BaiduCommon
{
	...
	function create_baifubao_pay_order_url($params, $url) {
		...
		if (!in_array($url, 
				array (
					C('BAIDU_PAY_DIRECT_LOGIN_URL'),
					C('BAIDU_PAY_DIRECT_NO_LOGIN_URL'),
					C('BAIDU_PAY_WAP_DIRECT_URL') 
				))) {
			$this->log(
	...
	function check_bfb_pay_result_notify() {
		...
		if (C('BAIDU_SP_NO') != $arr_params ['sp_no']) {
		...
		// 检查支付通知中的支付结果是否为支付成功
		if (C('BAIDU_PAY_RESULT_SUCCESS') != $arr_params ['pay_result']) {
...
// 		$this->log('baifubao query pay result by order_no success');
		...
		if (C('BAIDU_SP_PAY_RESULT_WAITING') == $order_state) {
			$this->log('the order state is right, the order is waiting for pay');
			return true;
		} elseif (C('BAIDU_SP_PAY_RESULT_SUCCESS') == $order_state) {
	...
	private function query_order_state($order_no) {
		 ...
		 */
		return C('BAIDU_SP_PAY_RESULT_WAITING');
	}
	...
	function query_baifubao_pay_result_by_order_no($order_no) {
		$params = array (
			'service_code' => C('BAIDU_QUERY_INTERFACE_SERVICE_ID'), // 查询接口的服务ID号
			'sp_no' => C('BAIDU_SP_NO'),
			'order_no' => $order_no,
			'output_type' => C('BAIDU_INTERFACE_OUTPUT_FORMAT'), // 百付宝返回XML格式的结果
			'output_charset' => C('BAIDU_INTERFACE_ENCODING'), // 百付宝返回GBK编码的结果
			'version' => C('BAIDU_INTERFACE_VERSION'),
			'sign_method' => C('BAIDU_SIGN_METHOD_MD5')
		);
		...
		$query_url = C('BAIDU_QUERY_ORDER_URL') . '?' . $params_str;
		$this->log(
				sprintf('the url of query baifubao pay result is [%s]', 
						$query_url));
		$content = $this->request($query_url);
		$retry = 0;
		while (empty($content) && $retry < C('BAIDU_QUERY_RETRY_TIME')) {
			$content = $this->request($query_url);
			$retry++;
		}
		if (empty($content)) {
		...
		if (C('BAIDU_SP_NO') != $response_arr ['sp_no']) {
			$this->log(
		...
		if (C('BAIDU_PAY_RESULT_SUCCESS') != $response_arr ['pay_result']) {
			$this->log(
					sprintf(
	...
	private function make_sign($params) {
		if (is_array($params)) {
			...
			if (ksort($params)) {
				...
				// 选择相应的加密算法
				if ($params ['sign_method'] == C('BAIDU_SIGN_METHOD_MD5')) {
					return md5($sign_str);
				} else if ($params ['sign_method'] == C('BAIDU_SIGN_METHOD_SHA1')) {
	...
	function log($msg) {
		...
		else
		{
			error_log(
					sprintf("[%s] [order_no: %s] : %s\n", date("Y-m-d H:i:s"), 
							$this->order_no, $msg), 3, C('BAIDU_LOG_FILE'));
		}
	}
}
