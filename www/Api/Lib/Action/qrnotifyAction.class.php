 ...
 *  提交、异步通知地址
 */
class qrnotifyAction extends baseAction
{
    ...
    *   付款成功通知
    */
    public function qrcode($sn='')
    {		
		eblog("民生一码付异步 - post数据",$_REQUEST,'scan_'.date("Ymd"));
		$url = 'http://172.16.102.101:8880/SinglePay/decodeAES';
		$postData = array(
			'content' => $_REQUEST['CONTENT'],		
			'encKey' => $this->KEY    
		);  
		eblog("民生一码付异步 - 解密数据",$postData,'qrnotify_'.date("Ymd"));
		$ansData = $this->httpGet($url, $postData); 
		eblog("民生一码付异步 - 返回解密数据",$ansData,'qrnotify_'.date("Ymd"));
		
        $data = explode("|",$ansData);
        //$data = json_decode($data, true);
		
		$transStatus = $data[1];
        $sn = $data[0];
		eblog('','=======================================','qrnotify_'.date("Ymd"));
		eblog("民生一码付异步接收参数集",$this->_request(),'qrnotify_'.date("Ymd"));
		if ($transStatus == '00')
		{
			//验证签名
			$sign = $_REQUEST['SIGN'];
			...
			if ($result == $sign)
			{
				...
				if(!$order)
				{
					eblog("民生一码付异步 - {$sn}",'订单不存在','qrnotify_'.date("Ymd"));
					return false;
				}
				$is_pay = $this->loadModel('order')->checkPay($order);
				if($is_pay)
				{
					eblog("民生一码付异步 - {$sn}",'订单已支付','qrnotify_'.date("Ymd"));
					echo '00';
					return false;
				}
				#判断状态为0的时候修改订单信息
				...
				#失败处理
				if(!$rsFTF){
				    eblog("民生一码付支付异步 - {$sn}",'重复订单号','qrnotify_'.date("Ymd"));
				    echo '00';
				    return false;
				}
				$rs =$this->loadModel('order')->pay($order);
				eblog("民生一码付异步- {$sn}--order",$order,'qrnotify_'.date("Ymd"));
				if ($rs)
				{
					eblog("民生一码付异步 - {$sn}",'订单支付成功','qrnotify_'.date("Ymd"));
					//发送异步交易查询,并发送相关消息
					A('Api://processMsg')->msg('TradeMsg',$sn);
				}
				else 
				{
					eblog("民生一码付异步 - {$sn}",'订单支付失败----','qrnotify_'.date("Ymd"));
					echo '00';
					return false;
				}
			...
			{
				eblog("民生一码付异步 - {$sn}",'签名验证失败','qrnotify_'.date("Ymd"));
			}
		}
	...
	public function cash($ptid,$money,$usid,$ctid = 10)
	{
		...
		eblog("民生扫描支付 - ",'提现数据'.$ptid.'--'.$money.'--'.$usid.'--'.$lfid,'scan_'.date("Ymd"));
		//提现通道限额检测
		$result = $this->loadModel('userSaler')->chkRealtimeLimit($usid,$ptid,$ctid,$lfid,$money);
		eblog("民生扫描支付 - ",'提现数据'.$result,'scan_'.date("Ymd"));
		if (!is_array($result))
		{
			eblog("民生扫描支付 - ",'提现通道限额检测'.$result,'scan_'.date("Ymd"));
			return false;
	...
	public function  single_pay($money,$serialNo,$dataUser = array()) {          
        ...
        if($money < 0)
        {
        	eblog("民生扫描支付- ",'代付金额小于0','single_pay_'.date("Ymd"));
        	return false;
        }
        $url = SINGLE_PAY_URL;
        $postData = array(
        ...
        eblog("民生扫描支付 - 代付数据",$postData,'single_pay_'.date("Ymd"));
        $ansData = $this->httpGet($url, $postData); 
    ...
    function callback($respContent) {
        ...
        $serialNo = $data['SEQ'];   
        eblog("民生扫描支付 - 代付返回数据",$data,'single_pay_'.date("Ymd"));     
        if($data['RES_CODE'] == '00')
        {
        	if ($serialNo)
			{
