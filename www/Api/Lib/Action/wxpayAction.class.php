...
class wxpayAction extends baseAction
{
    ...
    public function scan_pay()
    {
        ...
        eblog("微信支付 - fee_rate",$fee,'wxpay_'.date("Ymd"));
        ...
        $array = array($order['sn'],$ORDER_AMT,$USERFEE,'1','D0',$user['card_no'],$user['bank_no'],$user['name'],'微信支付');
        $CONTENT = implode("|", $array);//内容
        //AES加密
        ...
        eblog("微信获取二维码返回结果",$ansData,'wxpay_'.date("Ymd"));
        ...
        eblog("微信获取二维码返回",'订单内容：'.$result['CONTENT'][0].'|'.$result['CONTENT'][1].'|'.$result['CONTENT'][2].'|'.$result['CONTENT'][3].',订单金额：'.$result['MONEY'],'wxpay_'.date("Ymd"));


        if ($result['RES_CODE'] !== '0000') {
            eblog("微信获取二维码返回",'RES_CODE='.$result['RES_CODE'],'wxpay_'.date("Ymd"));
            $this->response(DATA_IMAGE,"错误: 交易失败");
        }
        if ($result['SIGN']!==$SIGN){
            eblog("微信获取二维码返回-签名不一致",'SIGN='.$SIGN.',RESULT_SING='.$result['SIGN'],'wxpay_'.date("Ymd"));
            $this->response(INTERNAL_ERROR,"错误: 交易失败");
        }
        if($result['ORDER_ID'] !== $order['sn']){
            eblog("微信获取二维码返回-订单号不一致",'sn='.$order['sn'].',RESULT_ORDER_ID='.$result['ORDER_ID'],'wxpay_'.date("Ymd"));
            $this->response(DATA_IMAGE,"错误: 订单号出错");
        }
        if( $result['MONEY'] != $ORDER_AMT){
            eblog("微信获取二维码返回-订单金额不一致",'ORDER_AMT='.$ORDER_AMT.',RESULT_MONEY='.$result['MONEY'],'wxpay_'.date("Ymd"));
            $this->response(DATA_IMAGE,"错误: 订单金额出错");
        }
        if ($result['QRCODE_URL']==''){
            eblog("微信获取二维码返回",'QRCODE_URL='.$result['QRCODE_URL'],'wxpay_'.date("Ymd"));
            $this->response(DATA_IMAGE,"错误: 获取二维码失败");
        }

        $userInfo = $this->loadModel('userSaler')->getInfo('name', array('id' => $this->usid));
        $data['share_title'] = '扫一扫,轻松付款';
        $data['share_desc'] = '商户:' . $userInfo['name'] . '向您发起一笔金额为￥' . $order['money'] . '的收款';
        $data['price'] = $order['money'];
        $data['img_url'] = $result['QRCODE_URL'];
            ...
            if ($notify->data['result_code'] == 'SUCCESS' || $wxdebug === true)
            {
                ...
                if ($order)
                {
                    ...
                    else
                    {
                        ...
                        #失败处理
                        if(!$rsFTF){
                            eblog("微信异步 - {$sn}",'重复订单号','wxpay_'.date("Ymd"));
                            echo '00';
                            return false;
                        }
