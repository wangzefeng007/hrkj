<?php
class ceshipayAction extends baseAction
{
    function wap_pay()
    {
        $code = I('code');
    ...
    public function scan_pay()
    {
        ...
        if ($order)
        {
            $url = 'http://172.16.102.101:8880/SinglePay/scancode';
            $url = ALIPAY_SCAN_URL;
            ...
            $CONTENT = $postData['ORDER_ID'].'|'.$postData['ORDER_AMT']. '|'.$postData['PAY_CHANNEL'].'|'.$postData['ORDER_NAME'];
            $VERSION = "1.0";
            eblog("支付宝支付 - POST数据",$postData,'alipay_'.date("Ymd"));

            //支付平台订单号|客户订单号|金额|二维码|备注
            $ansData = $this->httpGet($url, $postData);
            $ansData['CONTENT'] = explode('|',$ansData['CONTENT']);
            $result['RES_CODE'] = $ansData['RES_CODE'];
            $result['ORDER'] = $ansData['CONTENT'][0];
            $result['ORDER_ID'] = $ansData['CONTENT'][1];
            $result['MONEY'] = $ansData['CONTENT'][2];
            $result['QRCODE_URL'] = $ansData['CONTENT'][3];
            var_dump($result);exit;
            eblog("支付宝支付 - 返回数据",$result,'alipay_'.date("Ymd"));
            //$result['returnMsg']=urldecode($result['returnMsg']);
            if ($result['RES_CODE'] == '00')
            {
                if($result['ORDER_ID'] == $order['sn'])
                {
                    if ($result['MONEY'] == $order['money']){
                        ...
                        /*getQrcodePayUrl(array('amount'=>$order['money'],'url'=>$result['QRCODE_URL']),0);*/
                        //getQrcodePayUrl(array('amount'=>$order['money'],'url'=>$result['QRCODE_URL']),0);
                        $this->apiOut($data);
                    }
                    else{
        ...
        {
            eblog("支付宝支付- {$sn}",'订单不存在','alipay_'.date("Ymd"));
            $this->apiOut($data);
        }
    }
    ...
    */
    public function payNotice($sn='')
    {
        ...
        if ($sn == '' && $sn)
        {
            ...
            eblog("支付宝异步 - {$sn} - 人工post数据",$_params,'wxpay_'.date("Ymd"));
        }
        else
        {
            ...
            // eblog("支付宝异步接收参数集-xml",$xml,'wxpay_'.date("Ymd"));
    ...
    public function  single_pay($money,$serialNo,$dataUser = array()) {
        ...
        $fee = $this->getfee($dataUser);
        if($fee){
            $requestData['userfee'] = strval($fee['fee_rate']*$money + $fee['fee_static']);
        }
        eblog("银联APP异步 - 代付数据",$postData,'test_'.date("Ymd"));
        $ansData = $this->httpGet($url, $postData);
        return $this->callback($ansData);
    }
    public function getfee($dataUser){
        $usinfo = $this->loadModel('userSaler')->getInfoByid($dataUser['order']['usid']);
        $lfid = $usinfo['lfid'];
        //eblog("银联WAP支付异步 - ",'提现数据'.$ptid.'--'.$money.'--'.$usid.'--'.$lfid,'upacp_'.date("Ymd"));
        //提现通道限额检测
        $result = $this->loadModel('userSaler')->chkRealtimeLimit($dataUser['order']['usid'],$dataUser['payinfo']['ptid'],10,$lfid,$dataUser['order']['money']);
        if (!is_array($result))
        {
